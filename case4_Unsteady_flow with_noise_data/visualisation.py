...
import numpy as np
model = PINN()
model.net.load_state_dict(torch.load(f'model_with{noise}noise.param'))
print(f"Source Strength (lambda_m): {model.lambda_m.item():.4f}")
print(f"Source Location (lambda_x): {model.lambda_x.item():.4f}")
...
gs = gridspec.GridSpec(2, 3, height_ratios=[1, 1], width_ratios=[1, 1, 1], top=0.96,
bottom=0.065,
left=0.05,
right=0.99,
hspace=0.3,
...
ax_error.set_title(r"log$_{10}$ (C$_{pinn}$ - C$_{CFD}$)", fontsize=15)
ax_error.set_xlabel("x(m)", fontsize=15)
ax_error.set_ylabel("t(s)", fontsize=15)
...
ax2.set_ylabel("t(s)", fontsize=15)
...
ax_curve1.text(8.0, y_value_to_mark, f't={t_max * 0.2:.1f}s', color='black', fontsize=11, ha='center', va='center')
ax_curve1.text(13.52, y_value_to_mark, f't={t_max * 0.4:.1f}s', color='black', fontsize=11, ha='center', va='center')
ax_curve1.text(17.41, y_value_to_mark, f't={t_max * 0.6:.1f}s', color='black', fontsize=11, ha='center', va='center')
ax_curve1.text(19.29, 0.25, f't={t_max * 0.8:.1f}s', color='black', fontsize=11, ha='center', va='center')
ax_curve1.text(19.29, 0.4, f't={t_max * 1.0:.1f}s', color='black', fontsize=11, ha='center', va='center')

ax_curve1.set_title('(d)Concentration changes at different times', fontsize=15 )
ax_curve1.set_xlabel('x(m)', fontsize=15)
ax_curve1.set_ylabel(r"C(g/m$^{-3}$)", fontsize=15)
...
ax_curve2.text(2.14, 0.20, f'Section_{x_max * 0.4:.1f}m', color='black', fontsize=11, ha='center', va='center')
ax_curve2.text(3.47, 0.15 ,f'Section_{x_max * 0.6:.1f}m', color='black', fontsize=11, ha='center', va='center')
ax_curve2.text(5.08, 0.20, f'Section_{x_max * 0.8:.1f}m', color='black', fontsize=11, ha='center', va='center')
ax_curve2.text(7.96, 0.2, f'Section_{x_max * 1.0:.1f}m', color='black', fontsize=11, ha='center', va='center')


ax_curve2.set_title('(e)Concentration changes at different sections', fontsize=15 )
ax_curve2.set_xlabel('t(s)', fontsize=15)
ax_curve2.set_ylabel(r"C(g/m$^{-3}$)", fontsize=15)
