...
def fractional_derivative(net, x, t, alpha=1.5, h=0.05, max_k=100, direction='backward'):
    """修改后的分数阶导数实现"""
...
class PINN:
    def __init__(self):
        ...
        self.lambda_m.requires_grad_(True)
        self.lambda_x.requires_grad_(True)
        # print(self.lambda_m)
        # print(self.lambda_x)
    ...
    def loss_PDE(self):
        ...
        d15c_dx15 = fractional_derivative(self.net, x, t, alpha=1.5,
                                          h=0.05, max_k=100,
                                          direction='backward')
        ...
        # PDE残差
        pde = dc_dt + u * dc_dx- self.diffusion_x * d15c_dx15 - S
    ...
    def closure(self):
        ...
        loss = loss_ic + loss_bc + 10*loss_pde + loss_obs
...
if __name__ == "__main__":
    ...
    torch.save(pinn.net.state_dict(), f'modelb2.param')
    # 将记录的数据保存到 Excel
    df = pd.DataFrame(pinn.recorded_data)
    df.to_excel("training_log_b2.xlsx", index=False)